...
const ATTR_CONFIG: Record<string, AttrConfig[]> = {
  ...
  'pequenos electrodomesticos': [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/robot.*cocina|thermomix|monsieur/.test(s)) return 'Robot de cocina'; if (/tostador/.test(s)) return 'Tostador'; if (/aspirador|aspira/.test(s)) return 'Aspirador'; if (/batidora|batidor/.test(s)) return 'Batidora'; if (/freidora|air\s*fry/.test(s)) return 'Freidora de aire'; if (/cafetera/.test(s)) return 'Cafetera'; if (/microonda/.test(s)) return 'Microondas'; if (/plancha/.test(s)) return 'Plancha'; if (/sandwichera|gofrera|waffle/.test(s)) return 'Sandwichera'; return null; } },
  ],
  'pequenos electrodomestico': [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/robot.*cocina|thermomix|monsieur/.test(s)) return 'Robot de cocina'; if (/tostador/.test(s)) return 'Tostador'; if (/aspirador|aspira/.test(s)) return 'Aspirador'; if (/batidora|batidor/.test(s)) return 'Batidora'; if (/freidora|air\s*fry/.test(s)) return 'Freidora de aire'; if (/cafetera/.test(s)) return 'Cafetera'; if (/microonda/.test(s)) return 'Microondas'; if (/plancha/.test(s)) return 'Plancha'; if (/sandwichera|gofrera|waffle/.test(s)) return 'Sandwichera'; return null; } },
  ],
  'robot de cocina': [
    { label: 'Capacidad', extract: (n) => { const m = n.match(/(\d+(?:[.,]\d+)?)\s*[lL](?:\b|$)/); return m ? `${m[1]} L` : null; } },
  ],
  'robot de cocinas': [
    { label: 'Capacidad', extract: (n) => { const m = n.match(/(\d+(?:[.,]\d+)?)\s*[lL](?:\b|$)/); return m ? `${m[1]} L` : null; } },
  ],
  batidora: [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/vaso|jarra/.test(s)) return 'De vaso'; if (/amasadora/.test(s)) return 'Amasadora'; if (/varilla|manual/.test(s)) return 'De mano'; if (/robot|kitchen/.test(s)) return 'Robot'; return null; } },
    { label: 'Potencia', extract: (n) => { const m = n.match(/(\d{3,4})\s*[wW](?:\b|$)/); return m ? `${m[1]} W` : null; } },
  ],
  batidoras: [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/vaso|jarra/.test(s)) return 'De vaso'; if (/amasadora/.test(s)) return 'Amasadora'; if (/varilla|manual/.test(s)) return 'De mano'; if (/robot|kitchen/.test(s)) return 'Robot'; return null; } },
  ...
  freidora: [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/air|aire|sin\s*aceite/.test(s)) return 'Sin aceite'; if (/aceite|tradicional/.test(s)) return 'Con aceite'; return null; } },
    { label: 'Capacidad', extract: (n) => { const m = n.match(/(\d+(?:[.,]\d+)?)\s*[lL](?:\b|$)/); return m ? `${m[1]} L` : null; } },
  ],
  freidoras: [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/air|aire|sin\s*aceite/.test(s)) return 'Sin aceite'; if (/aceite|tradicional/.test(s)) return 'Con aceite'; return null; } },
  ...
  cafetera: [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/capsul|nespresso|dolce/.test(s)) return 'Cápsulas'; if (/espresso|express/.test(s)) return 'Espresso'; if (/goteo|filtro/.test(s)) return 'Goteo'; if (/italiana|moka/.test(s)) return 'Italiana'; if (/superautom|super\s*auto/.test(s)) return 'Superautomática'; return null; } },
  ],
  cafeteras: [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/capsul|nespresso|dolce/.test(s)) return 'Cápsulas'; if (/espresso|express/.test(s)) return 'Espresso'; if (/goteo|filtro/.test(s)) return 'Goteo'; if (/italiana|moka/.test(s)) return 'Italiana'; if (/superautom|super\s*auto/.test(s)) return 'Superautomática'; return null; } },
  ],
  'olla electrica': [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/presi[oó]n/.test(s)) return 'A presión'; if (/lenta|slow/.test(s)) return 'Cocción lenta'; if (/arrocera|arroz/.test(s)) return 'Arrocera'; return null; } },
    { label: 'Capacidad', extract: (n) => { const m = n.match(/(\d+(?:[.,]\d+)?)\s*[lL](?:\b|$)/); return m ? `${m[1]} L` : null; } },
  ],
  'ollas electricas': [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/presi[oó]n/.test(s)) return 'A presión'; if (/lenta|slow/.test(s)) return 'Cocción lenta'; if (/arrocera|arroz/.test(s)) return 'Arrocera'; return null; } },
  ...
  'plancha de cocina': [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/grill|grila/.test(s)) return 'Grill'; if (/sandwi|sandwi/.test(s)) return 'Sandwichera'; if (/gofre|waffle/.test(s)) return 'Gofrera'; if (/pl[aá]ncha/.test(s)) return 'Plancha lisa'; return null; } },
  ],
  'planchas de cocina': [
    { label: 'Tipo', extract: (n) => { const s = n.toLowerCase(); if (/grill/.test(s)) return 'Grill'; if (/sandwi/.test(s)) return 'Sandwichera'; if (/gofre|waffle/.test(s)) return 'Gofrera'; if (/pl[aá]ncha/.test(s)) return 'Plancha lisa'; return null; } },
  ],
};
    ...
    <>
      ...
      <div className="flex gap-6 items-start">
        ...
        <aside className="hidden lg:flex flex-col w-56 shrink-0 sticky top-[90px] bg-white rounded-2xl border border-neutral-200 shadow-sm overflow-hidden">
          ...
          {/* Secciones de filtro por atributo (colapsables, cerradas por defecto) */}
          {attrConfigs.map((cfg) => {
            const vals = getAttrValues(cfg, itemsInCategory);
            if (vals.length < 2) return null;
            const selected = selectedAttrs[cfg.label] || '';
            const isOpen = !!openSections[cfg.label];
            return (
              <div key={cfg.label} className="border-t border-neutral-100">
                <button
                  onClick={() => toggleSection(cfg.label)}
                  className="w-full flex items-center justify-between px-4 py-3 hover:bg-neutral-50 transition-colors"
                >
                  <span className="text-xs font-semibold text-neutral-500 uppercase tracking-wider">{cfg.label}</span>
                  <svg className={`w-3.5 h-3.5 text-neutral-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="m6 9 6 6 6-6"/></svg>
                </button>
                {isOpen && (
                  <ul className="pb-2">
                    <li>
