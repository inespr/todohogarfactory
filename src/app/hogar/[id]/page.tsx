...
export default function HogarDetailPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const snap = await getDoc(doc(db, 'hogar', id));
        if (!snap.exists()) {
          setNotFound(true);
        } else {
  ...
  const subcat = product.subcategoria || (
    product.category && product.category.toLowerCase() !== 'hogar'
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <nav className="flex flex-wrap items-center gap-1.5 text-sm text-neutral-400 mb-6">
        ...
        <span>›</span>
        <Link href="/hogar" className="hover:text-neutral-700 transition-colors">Hogar</Link>
        {subcat && (
          <>
            <span>›</span>
            <Link href={`/hogar?subcategory=${encodeURIComponent(subcat.toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '').replace(/\s+/g, '-'))}`} className="hover:text-neutral-700 transition-colors">
              {subcat}
            </Link>
          </>
        )}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="relative bg-white rounded-2xl border border-neutral-200 overflow-hidden" style={{ height: '480px' }}>
            ...
            <img
              ...
              onError={(e) => { (e.target as HTMLImageElement).src = '/placeholders/hogar.svg'; }}
            />
            {product.hasDefect && (
